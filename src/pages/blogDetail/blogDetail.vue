...
export default {
  ...
  beforeMount() {
    this.detailObj = this.blogTmp[this.$mp.query.index];
    // 看收藏状态
    wx.request({
  ...
  methods: {
    ...
    formSubmit(e) {
      ...
      if (this.$store.state.user.length == 0) {
        ...
        wx.request({
          url: "http://localhost:8080/Comment/publish", //开发者服务器接口地址",
          data: {
            uid: this.$store.state.user.user_id,
            bid: this.detailObj.blog.blog_id,
            content: e.mp.detail.value.comment,
          }, //请求的参数",
          method: "GET",
          dataType: "json", //如果设为json，会尝试对返回的数据做一次 JSON.parse
          success: (res) => {
